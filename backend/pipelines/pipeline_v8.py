...
class AtlanticDigitalPipeline:
            ...
            for schema in comp.schema_data:
                if isinstance(schema, dict):
                    for key in ['name', 'legalName', 'alternateName']:
                        org_name = schema.get(key, '')
                        if org_name and isinstance(org_name, str):
                            brand_terms.add(org_name.lower())
        ...
        try:
            ...
            def _parse_rkw_items(items, boost=1.0):
                for item in items:
                    ...
                    vol = (kw_data.get('keyword_info') or {}).get('search_volume', 0) or 0
                    if kw and vol > 0 and not self._is_brand_keyword(kw):
            ...
            for lang, loc in [("es", 2724), ("en", 2724)]:
                ...
                try:
                    r = requests.post(endpoint_kfs, headers=headers, timeout=30, json=[{
    ...
    def run_analysis(self, client_url: str, competitor_urls: List[str],
        ...
        print("\n" + "="*60)
        print("🚀 ATLANTIC DIGITAL SEO/GEO ANALYSIS v6 (FIXED)")
        print("="*60)
    ...
    def _analyze_seo_comprehensive(self, content: ScrapedContent,
        ...
        bigrams_filtered = [b for b in bigrams if not any(w in STOP_WORDS or w in WEB_NOISE or self._is_brand_keyword(w) for w in b.split())]
        top_bigrams = [(b, c) for b, c in Counter(bigrams_filtered).most_common(20) if c > 1 and b.split()[0] != b.split()[1]][:15]

        trigrams = [' '.join(tokens[i:i + 3]) for i in range(len(tokens) - 2)]
        trigrams_filtered = [t for t in trigrams if not any(w in STOP_WORDS or w in WEB_NOISE or self._is_brand_keyword(w) for w in t.split())]
        ...
        if soup:
            for link in soup.find_all('a', href=True):
                if link['href'].startswith('http') and urlparse(content.url).netloc not in link['href']:
                    external_links += 1
                else:
                    internal_links += 1
    ...
    def _competitive_analysis(self, client_geo: GEOMetrics, client_seo: SEOMetrics,
        ...
        try:
            ...
            return {
                ...
                'position': 'competitive' if np.mean(sims) > 0.5 else 'differentiated'
            }
        except:
            return {'similarity': 0.5, 'position': 'unknown'}
