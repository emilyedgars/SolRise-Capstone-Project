...
const GeneratePanel = ({ results, info, projectId }) => {
    ...
    // Load saved HTML from MongoDB on mount
    useEffect(() => {
        if (!projectId) return;
        fetch(`/api/project/${projectId}`)
            ...
            .then(data => {
                if (data.generated_html) {
                    ...
                    if (data.generation_validation) {
                        setStats({
                            score: data.generation_validation.score ?? 0,
                            method: data.generation_method ?? 'saved',
                            time: data.generation_time ?? null,
                        });
                    }
                }
            })
    ...
    const generate = async () => {
        ...
        try {
            ...
            setHtml(data.html);
            setStats({ score: data.validation?.score ?? 0, method: data.method ?? '-', time: data.generation_time });
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        {stats && (
                            <div style={{ background: 'rgba(247, 161, 79, 0.2)', color: '#F7A14F', padding: '0.5rem 1rem', borderRadius: 20, fontSize: '0.85rem' }}>
                                ✓ Score: {stats.score}% · {stats.method} · {stats.time}s
                            </div>
                        )}
                    </div>
                    {improvements.length > 0 && (
                        <div style={{ background: 'rgba(247,161,79,0.06)', border: '1px solid rgba(247,161,79,0.2)', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
                            <div style={{ color: '#F7A14F', fontWeight: 600, fontSize: '0.85rem', marginBottom: '0.6rem' }}>
                                ✅ {improvements.length} Improvements Applied
                            </div>
                            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '0.35rem' }}>
                                {improvements.map((imp, i) => (
                                    <li key={i} style={{ fontSize: '0.8rem', color: '#bdc3c7', display: 'flex', alignItems: 'flex-start', gap: '0.4rem' }}>
                                        <span style={{ color: '#F7A14F', flexShrink: 0 }}>✓</span>{imp}
                                    </li>
                                ))}
                            </ul>
                        </div>
