...
class AtlanticDigitalPipeline:
    ...
    def _extract_brand_terms(self, competitors: List[ScrapedContent]) -> Set[str]:
        """Extract brand names from competitor data to exclude from keywords"""
        brand_terms = set()
        ...
        for comp in competitors:
            ...
            try:
                ...
                if len(domain_parts) >= 2:
                    brand_terms.add(domain_parts[0])
            except:
                pass
                
            # 2. Extract from title (usually "Brand Name - Tagline" or "Tagline | Brand Name")
            if comp.title:
                title_parts = re.split(r'[|\-:]', comp.title)
                # Usually the brand is at the start or end
                if len(title_parts) > 1:
                    candidate1 = title_parts[0].strip().lower()
                    candidate2 = title_parts[-1].strip().lower()
                    
                    # Heuristic: Brand names are usually short (1-3 words)
                    if 0 < len(candidate1.split()) <= 3:
                        brand_terms.add(candidate1)
                    if 0 < len(candidate2.split()) <= 3:
                        brand_terms.add(candidate2)
                        
        print(f"   🚫 Discovered competitor brand terms to ignore: {brand_terms}")
        return brand_terms
    ...
    def _fetch_dataforseo_keywords(self, url: str) -> List[Tuple[str, int]]:
        ...
        if not self.dfs_login or not self.dfs_password:
            print("   ⚠️ DataForSEO credentials not found. Using local extraction.")
            return []
            
        print(f"   🌍 Querying DataForSEO for {url}...")
        ...
        try:
            ...
            # Endpoint: Google Keywords for Site (Live)
            ...
            if response.status_code != 200:
                print(f"   ⚠️ DataForSEO API Error: {response.status_code} {response.text[:100]}")
                return []
            ...
            for item in items:
                ...
                vol = item.get('keyword_data', {}).get('search_volume', 0)
                if kw:
            ...
            print(f"   ⚠️ DataForSEO Exception: {str(e)}")
            return []
    ...
    def run_analysis(self, client_url: str, competitor_urls: List[str], 
        ...
        # Step 1b: Extract Competitor Brand Terms
        if competitor_data:
            ...
            print(f"   Updated brand filter: {self.brand_terms}")
    ...
    def _analyze_seo_comprehensive(self, content: ScrapedContent, 
        ...
        # Merge local TF-IDF keywords with External DataForSEO keywords
        local_keywords = word_freq.most_common(30)
        
        final_keywords = []
        seen_keywords = set()
        
        # 1. Add DataForSEO keywords (High Priority)
        if external_keywords:
            for kw, vol in external_keywords:
                kw_lower = kw.lower()
                # Filter brand terms from DataForSEO too
                if not any(b in kw_lower for b in brand_parts) and kw_lower not in seen_keywords:
                    final_keywords.append((kw, vol)) # Volume from API
                    seen_keywords.add(kw_lower)
        
        # 2. Add Local TF-IDF keywords (if not already present)
        for kw, count in local_keywords:
            kw_lower = kw.lower()
            if kw_lower not in seen_keywords:
                # Filter out terms that look like brand names (heuristic: matches brand parts exactly)
                if not any(b == kw_lower for b in brand_parts):
                     final_keywords.append((kw, count * 10)) # Boost local count to compare with volume
                     seen_keywords.add(kw_lower)
